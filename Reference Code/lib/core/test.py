...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        if cfg.TEST.MASK_AUG.ENABLED:
            ...
            blob_conv, _, _, _ = im_conv_body_and_branch_feature(model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE)
            
            amodal_feature, class_feature = im_conv_body_and_branch_feature_again(model, blob_conv, im, boxes, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE)
            #pdb.set_trace()
            #class_feature_extract = class_feature[return_keep_np, :, :, :]
            #amodal_feature_extract = amodal_feature[return_keep_np, :, :, :]

            class_feature_extract = class_feature
            amodal_feature_extract = amodal_feature
...
def im_conv_body_and_branch_feature_again(model, blob_conv, im, boxes, target_scale, target_max_size):
    #boxes = None
    inputs, im_scale = _get_blobs(im, boxes, target_scale, target_max_size)
    conv_body_only = np.ones(1, dtype=np.int) 
    if cfg.PYTORCH_VERSION_LESS_THAN_040:
    ...
    if cfg.FPN.MULTILEVEL_ROIS:
        _add_multilevel_rois_for_test(inputs, 'rois')
    amodal_feature, class_feature = model.module.det_net(blob_conv, inputs)
    # blob_conv, amodal_feature, class_feature = model(**inputs)

    return amodal_feature, class_feature
