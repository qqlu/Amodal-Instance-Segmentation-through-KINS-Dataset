...
def im_detect_mask_aug(model, im, boxes, blob_conv):
    ...
    if cfg.TEST.MASK_AUG.H_FLIP:
        masks_hf = im_detect_mask_hflip(model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, boxes, blob_conv)
...
def im_detect_mask_hflip(model, im, target_scale, target_max_size, boxes, blob_conv):
...
def im_detect_mask_scale(model, im, target_scale, target_max_size, boxes, blob_conv, hflip=False):
    """Computes masks at the given scale"""
    if hflip:
        masks_scl = im_detect_mask_hflip(model, im, target_scale, target_max_size, boxes, blob_conv)
    else:
...
def im_detect_mask_aspect_ratio(model, im, aspect_ratio, boxes, blob_conv, hflip=False):
    ...
    if hflip:
        masks_ar = im_detect_mask_hflip(model, im_ar, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, boxes_ar, blob_conv)
    else:
        _, im_scale, _ = blob_utils.get_image_blob(im, target_scale, target_max_size)
