...
from pycocotools.coco_amodalinmodalseg import COCO
...
class JsonDatasetAmodalInmodalSeg(object):
    ...
    def valid_cached_keys(self):
        ...
        """
        keys = ['boxes_amodal', 'boxes_inmodal', 'segms_amodal', 'segms_inmodal', 'gt_classes', 'amodal_label', 'segms_amodal_areas', 'gt_overlaps',
                'is_crowd', 'box_to_gt_ind_map', 'segms_invisible_full']
    ...
    def _prep_roidb_entry(self, entry):
        ...
        # Empty placeholders
        # boxes, segms
        entry['boxes_amodal'] = np.empty((0, 4), dtype=np.float32)
        entry['boxes_inmodal'] = np.empty((0, 4), dtype=np.float32)
        entry['segms_amodal'] = []
        entry['segms_inmodal'] = []
        entry['segms_invisible_full'] = []

        # classification
        entry['gt_classes'] = np.empty((0), dtype=np.int32)
        entry['amodal_label'] = np.empty((0), dtype=np.int32)

        entry['segms_amodal_areas'] = np.empty((0), dtype=np.float32)
    ...
    def _add_gt_annotations(self, entry):
        ...
        # pdb.set_trace()
        # print(entry.keys())
        amodal_full_mask = np.array(mask_util.decode(entry['amodal_full']), dtype=np.float32)
        # print(entry['amodal_full'])
        # input()
        for obj in objs:
            ...
            # pdb.set_trace()
            temp_unique = np.unique(amodal_full_mask[int(x1):int(x2+1), int(y1):int(y2+1)])
            if len(temp_unique) == 2:
                amodal_label.append(1)
            else:
                amodal_label.append(0)
            
            x1_in, y1_in, x2_in, y2_in = box_utils.xywh_to_xyxy(obj['inmodal_bbox'])
            x1_in, y1_in, x2_in, y2_in = box_utils.clip_xyxy_to_image(x1_in, y1_in, x2_in, y2_in, height, width)
            # Require non-zero seg area and more than 1x1 box size
            if obj['area'] > 0 and x2 > x1 and y2 > y1:
                obj['clean_bbox'] = [x1, y1, x2, y2]
                obj['inmodal_bbox'] = [x1_in, y1_in, x2_in, y2_in]
                valid_objs.append(obj)
                valid_segms_amodal.append(obj['segmentation'])
                valid_segms_inmodal.append(obj['inmodal_seg'])

        num_valid_objs = len(valid_objs)

        boxes_amodal = np.zeros((num_valid_objs, 4), dtype=entry['boxes_amodal'].dtype)
        boxes_inmodal = np.zeros((num_valid_objs, 4), dtype=entry['boxes_amodal'].dtype)
        
        gt_classes = np.zeros((num_valid_objs), dtype=entry['gt_classes'].dtype)
        gt_overlaps = np.zeros((num_valid_objs, self.num_classes), dtype=entry['gt_overlaps'].dtype)
        seg_areas = np.zeros((num_valid_objs), dtype=entry['segms_amodal_areas'].dtype)
        is_crowd = np.zeros((num_valid_objs), dtype=entry['is_crowd'].dtype)
        box_to_gt_ind_map = np.zeros((num_valid_objs), dtype=entry['box_to_gt_ind_map'].dtype)

        amodal_label = np.array(amodal_label, dtype=entry['amodal_label'].dtype)
        if self.keypoints is not None:
        ...
        for ix, obj in enumerate(valid_objs):
            ...
            boxes_amodal[ix, :] = obj['clean_bbox']
            boxes_inmodal[ix, :] = obj['inmodal_bbox']
            gt_classes[ix] = cls
        ...
        entry['boxes_amodal'] = np.append(entry['boxes_amodal'], boxes_amodal, axis=0)
        entry['boxes_inmodal'] = np.append(entry['boxes_inmodal'], boxes_inmodal, axis=0)
        entry['segms_amodal'].extend(valid_segms_amodal)
        entry['segms_inmodal'].extend(valid_segms_inmodal)
        entry['segms_invisible_full'].append(entry['amodal_full'])

        # To match the original implementation:
        # entry['boxes'] = np.append(
        #     entry['boxes'], boxes.astype(np.int).astype(np.float), axis=0)
        entry['gt_classes'] = np.append(entry['gt_classes'], gt_classes)
        entry['amodal_label'] = np.append(entry['amodal_label'], amodal_label)

        entry['segms_amodal_areas'] = np.append(entry['segms_amodal_areas'], seg_areas)
    ...
    def _add_gt_from_cache(self, roidb, cache_filepath):
        ...
        for entry, cached_entry in zip(roidb, cached_roidb):
            ...
            boxes_amodal, boxes_inmodal, segms_amodal, segms_inmodal, \
            gt_classes, amodal_label, \
            segms_amodal_areas, \
            gt_overlaps, is_crowd, \
            box_to_gt_ind_map, segms_invisible_full = values[:11]
            if self.keypoints is not None:
                gt_keypoints, has_visible_keypoints = values[7:]
            entry['boxes_amodal'] = np.append(entry['boxes_amodal'], boxes_amodal, axis=0)
            entry['boxes_inmodal'] = np.append(entry['boxes_inmodal'], boxes_inmodal, axis=0)
            entry['segms_amodal'].extend(segms_amodal)
            entry['segms_inmodal'].extend(segms_inmodal)
            entry['segms_invisible_full'].extend(segms_invisible_full)
            # pdb.set_trace()
            # To match the original implementation:
            # entry['boxes'] = np.append(
            #     entry['boxes'], boxes.astype(np.int).astype(np.float), axis=0)
            ...
            entry['segms_amodal_areas'] = np.append(entry['segms_amodal_areas'], segms_amodal_areas)
...
def _merge_proposal_boxes_into_roidb(roidb, box_list):
    ...
    for i, entry in enumerate(roidb):
        ...
        amodal_full_mask = np.array(mask_util.decode(entry['segms_invisible_full']), dtype=np.float32)
        amodal_label = []
        if len(gt_inds) > 0:
            gt_boxes = entry['boxes_amodal'][gt_inds, :]
        ...
        entry['boxes_amodal'] = np.append(
            entry['boxes_amodal'],
            boxes.astype(entry['boxes_amodal'].dtype, copy=False),
            axis=0
        )
        ...
        entry['segms_amodal_areas'] = np.append(
            entry['segms_amodal_areas'],
            np.zeros((num_boxes), dtype=entry['segms_amodal_areas'].dtype)
        )
