...
from pycocotools.cocoeval_amodalinmodalseg import COCOeval
...
def evaluate_masks(
    ...
    if json_dataset.name.find('test') == -1:
        coco_eval = _do_segmentation_eval(json_dataset, res_file, output_dir, c_type)
    else:
        coco_eval = None
...
def _write_coco_segms_results_file_ctype(
    json_dataset, all_boxes, all_segms, res_file, c_type
):
    # [{"image_id": 42,
    #   "category_id": 18,
    #   "segmentation": [...],
    ...
    for cls_ind, cls in enumerate(json_dataset.classes):
        ...
        if cls_ind >= len(all_boxes):
            break
        cat_id = json_dataset.category_to_id_map[cls]
        results.extend(_coco_segms_results_one_category_ctype(
            json_dataset, all_boxes[cls_ind], all_segms[cls_ind], cat_id, c_type))
    logger.info(
        'Writing segmentation results json to: {}'.format(
            os.path.abspath(res_file)))
    with open(res_file, 'w') as fid:
...
def _coco_segms_results_one_category_ctype(json_dataset, boxes, segms, cat_id, c_type):
    results = []
    image_ids = json_dataset.COCO.getImgIds()
    image_ids.sort()
    assert len(boxes) == len(image_ids)
