...
def evaluate_all(
    dataset, all_boxes, all_segms_amodal, all_segms_inmodal, output_dir, use_matlab=False
):
    ...
    if cfg.MODEL.MASK_ON:
        results = evaluate_masks_amodal(dataset, all_boxes, all_segms_amodal, output_dir)
        all_results[dataset.name].update(results[dataset.name])
        logger.info('Evaluating segmentations amodal is done!')
    if cfg.MODEL.INMODAL_ON:
        results = evaluate_masks_inmodal(dataset, all_boxes, all_segms_inmodal, output_dir)
        all_results[dataset.name].update(results[dataset.name])
        logger.info('Evaluating segmentations inmodal is done!')
    return all_results
...
def evaluate_masks_amodal(dataset, all_boxes, all_segms_amodal, output_dir):
    """Evaluate instance segmentation."""
    logger.info('Evaluating segmentations amodal')
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_json_dataset_evaluator(dataset):
        coco_eval_amodal = json_dataset_evaluator.evaluate_masks(
            dataset,
            all_boxes,
            all_segms_amodal,
            output_dir,
        ...
        mask_results_amodal = _coco_eval_to_mask_results(coco_eval_amodal)
    else:
        raise NotImplementedError(
            'No evaluator for dataset: {}'.format(dataset.name)
        )
    return OrderedDict([(dataset.name, mask_results_amodal)])

def evaluate_masks_inmodal(dataset, all_boxes, all_segms_inmodal, output_dir):
    """Evaluate instance segmentation."""
    logger.info('Evaluating segmentations inmodal')
    not_comp = not cfg.TEST.COMPETITION_MODE
    if _use_json_dataset_evaluator(dataset):
        coco_eval_inmodal = json_dataset_evaluator.evaluate_masks(
            dataset,
            all_boxes,
            all_segms_inmodal,
            output_dir,
        ...
        mask_results_inmodal = _coco_eval_to_mask_results(coco_eval_inmodal)
    else:
        raise NotImplementedError(
            'No evaluator for dataset: {}'.format(dataset.name)
        )
    return OrderedDict([(dataset.name, mask_results_inmodal)])
...
def _use_json_dataset_evaluator(dataset):
    ...
    return dataset.name.find('coco_') > -1 or dataset.name.find('common')>-1 or dataset.name.find('kitti')>-1 or cfg.TEST.FORCE_JSON_DATASET_EVAL
