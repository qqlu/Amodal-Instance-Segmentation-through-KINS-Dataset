...
from modeling.collect_and_distribute_fpn_rpn_proposals_panet import CollectAndDistributeFpnRpnProposalsOp
import nn as mynn
...
class fpn(nn.Module):
    ...
    def __init__(self, conv_body_func, fpn_level_info, P2only=False):
        ...
        self.panet_downtop_lateral_modules = nn.ModuleList()
        self.panet_posthoc_modules = nn.ModuleList()
        ...
        # panet structure
        for i in range(self.num_backbone_stages-1):
            self.panet_downtop_lateral_modules.append(
                panet_downtop_lateral_module(fpn_dim))
    ...
    def _init_weights(self):
        ...
        for child_m in self.children():
            if (not isinstance(child_m, nn.ModuleList) or
                not isinstance(child_m, topdown_lateral_module) or
                not isinstance(child_m[0], panet_downtop_lateral_module)):
    ...
    def forward(self, x):
        ...
        panet_inner_blobs = [fpn_output_blobs[-1]]
        for i in range(self.num_backbone_stages - 1):
            #print(panet_inner_blobs[-1].size())
            #print(fpn_output_blobs[-(i+2)].size())
            panet_inner_blobs.append(
                self.panet_downtop_lateral_modules[i](fpn_output_blobs[-(i+2)], panet_inner_blobs[-1])
                )
        #print(len(panet_inner_blobs))
        #input()
        panet_output_blobs = []
        for i in range(self.num_backbone_stages):
            panet_output_blobs.append(
                panet_inner_blobs[-(i+1)])

        if hasattr(self, 'maxpool_p6'):
            panet_output_blobs.insert(0, self.maxpool_p6(panet_output_blobs[0]))
        ...
        if self.P2only:
            # use only the finest level
            return panet_output_blobs[-1]
        else:
            # use all levels
            return panet_output_blobs
...
class panet_downtop_lateral_module(nn.Module):
    """Add a top-down lateral module."""
    def __init__(self, dim_in_lateral):
        ...
        if cfg.FPN.USE_GN:
            self.conv_lateral = nn.Sequential(
                nn.Conv2d(self.dim_in_lateral, self.dim_in_lateral, 3, stride=2, padding=1),
                nn.GroupNorm(net_utils.get_group_gn(self.dim_in_lateral), self.dim_in_lateral,
                             eps=cfg.GROUP_NORM.EPSILON)
            )
        else:
            self.conv_lateral = nn.Conv2d(self.dim_in_lateral, self.dim_in_lateral, 3, stride=2, padding=1)


        if cfg.FPN.USE_GN:
            self.posthoc = nn.Sequential(
                nn.Conv2d(dim_in_lateral, dim_in_lateral, 3, 1, 1, bias=False),
                nn.GroupNorm(net_utils.get_group_gn(dim_in_lateral), dim_in_lateral,
                    eps=cfg.GROUP_NORM.EPSILON)
                )
        else:
    ...
    def forward(self, top_blob, lateral_blob):
        # Lateral 1x1 conv
        lat = F.relu(self.conv_lateral(lateral_blob))
        fus = F.relu(self.posthoc(lat + top_blob))
