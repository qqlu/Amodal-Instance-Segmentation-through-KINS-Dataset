...
import modeling.MobileNetV2_imitateRes as MobileNetV2
...
# FPN with ResNet
# ---------------------------------------------------------------------------- #
def fpn_MobileNetV2_body():
    return fpn(
        MobileNetV2.MobileNetV2_body, 
        fpn_level_info_MobileNetV2_conv5()
        )


def fpn_MobileNetV2_p2only_body():
    return fpn(
        MobileNetV2.MobileNetV2_body, 
        fpn_level_info_MobileNetV2_conv5(),
        P2only=True
...
class fpn(nn.Module):
    ...
    similarly for fpn_level_info.dims: e.g [1280, 96, 32, 24]
    ...
    def forward(self, x):
        conv_body_blobs = [self.conv_body.feature1(x)]
        for i in range(1, self.conv_body.convX):
            conv_body_blobs.append(
                getattr(self.conv_body, 'feature%d' % (i+1))(conv_body_blobs[-1])
            )
...
def fpn_level_info_MobileNetV2_conv5():
    return FpnLevelInfo(
        blobs=('feature5', 'feature4', 'feature3', 'feature2'),
        dims=(1280, 96, 32, 24),
