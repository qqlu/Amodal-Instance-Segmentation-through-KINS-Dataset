...
from datasets import json_dataset_amodalinmodalseg as json_dataset
import roi_data.fast_rcnn_amodalinmodalseg
    ...
    def forward(self, inputs, roidb, im_info):
        ...
        if self.training:
            ...
            if not cfg.MODEL.INMODAL_ON:
                ...
                # pdb.set_trace()
                output_blob_names = roi_data.fast_rcnn_amodalinmodalseg.get_fast_rcnn_blob_names()
                blobs = {k: [] for k in output_blob_names}
                roi_data.fast_rcnn_amodalinmodalseg.add_fast_rcnn_blobs(blobs, im_scales, roidb)
        else:
