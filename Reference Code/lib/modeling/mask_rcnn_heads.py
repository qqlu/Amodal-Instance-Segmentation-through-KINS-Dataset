...
class mask_rcnn_outputs(nn.Module):
    ...
    def __init__(self, dim_in, amodal=False):
        super().__init__()
        self.dim_in = dim_in
        if not amodal:
...
class mask_rcnn_fcn_head_v1upXconvs_ff(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale, num_convs):
        ...
        # upsample fusion layer
        # self.ffconv_ac = nn.Sequential(nn.ConvTranspose2d(dim_inner*2, dim_inner, 2, 2, 0),
        #                                nn.ReLU(inplace=True))
        # self.ffconv_ac = nn.Sequential(nn.ConvTranspose2d(dim_inner*2, dim_inner, 2, 2, 0),
        #                                nn.ReLU(inplace=True),
        #                                nn.Conv2d(dim_inner, dim_inner, 3, 1, padding=1),
        #                                nn.ReLU(inplace=True)
        #                                )
        self.ffconv_ac = nn.Sequential(nn.Conv2d(dim_inner*2, dim_inner, 3, 1, padding=1),
                                       nn.ReLU(inplace=True),
        ...
        self.ffconv_acx = nn.Sequential(nn.Conv2d(dim_inner*2, dim_inner, 3, 1, padding=1),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(dim_inner, dim_inner, 3, 1, padding=1),
                                        nn.ReLU(inplace=True),
                                        )
    ...
    def forward(self, x, rpn_ret, amodal_feature, class_feature):
        x = self.roi_xform(
            x, rpn_ret,
            blob_rois='mask_rois',
            method=cfg.MRCNN.ROI_XFORM_METHOD,
        ...
        for i in range(self.num_convs):
            if i == cfg.MRCNN.FUSION_FEATURE:
                x_amodal_class = torch.cat((amodal_feature, class_feature), dim=1)
                x_ac = self.ffconv_ac(x_amodal_class)
                x_acx = torch.cat((x_ac, x), dim=1)
                x = self.ffconv_acx(x_acx)
                # pdb.set_trace()
            x = self.conv_fcn[i*2](x)
            x = self.conv_fcn[i*2+1](x)

        # x = self.conv_fcn(x)
        return F.relu(self.upconv(x), inplace=True)

class mask_rcnn_fcn_head_v1upXconvs_add(nn.Module):
    """v1upXconvs design: X * (conv 3x3), convT 2x2."""
    def __init__(self, dim_in, roi_xform_func, spatial_scale, num_convs):
        super().__init__()
        self.dim_in = dim_in
