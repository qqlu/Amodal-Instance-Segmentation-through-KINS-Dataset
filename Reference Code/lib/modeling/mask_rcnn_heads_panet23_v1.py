...
class mask_rcnn_outputs(nn.Module):
    ...
    def forward(self, x_fcn, x_fcf):
        x_fcn = self.classify(x_fcn)
        # x_fcf = x_fcf.repeat(1, self.n_classes, 1, 1)
        x_fcf = x_fcf.repeat(1, cfg.MODEL.NUM_CLASSES, 1, 1)
        x = x_fcn + x_fcf
        if cfg.MRCNN.UPSAMPLE_RATIO > 1:
            x = self.upsample(x)
...
class mask_rcnn_fcn_head_v1upXconvs(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale, num_convs):
        ...
        self.mask_fcn_1 = torch.nn.ModuleList()
        for i in range(num_convs):
            self.mask_fcn_1.append(nn.Sequential(
            nn.Conv2d(dim_in, dim_inner, 3, 1, padding=1*dilation, dilation=dilation),
        ))
            #self.mask_fcn_1.append(nn.Conv2d(dim_in, dim_inner, 3, 1, padding=1*dilation, dilation))
        dim_in = dim_inner
        ...
        self.last_conv = nn.Sequential(nn.Conv2d(dim_in, dim_in, 3, 1, padding=1*dilation, dilation=dilation),
                                       nn.ReLU(inplace=True))
        ...
        # panet
        self.mask_fcf_1 = nn.Conv2d(dim_inner, dim_inner, 3, stride=1, padding=1)
        self.mask_fcf_2 = nn.Conv2d(dim_inner, int(dim_inner / 2), 3, stride=1, padding=1)
        self.mask_fcf_3 = nn.Linear(int(dim_inner * cfg.MRCNN.ROI_XFORM_RESOLUTION * cfg.MRCNN.ROI_XFORM_RESOLUTION / 2), cfg.MRCNN.RESOLUTION ** 2)
    ...
    def forward(self, x, rpn_ret):
        ...
        feats = torch.FloatTensor(x[0].size()).fill_(0).cuda(device_id)
        ...
        # panet
        fcf = self.mask_fcf_1(x)
        fcf = self.mask_fcf_2(F.relu(fcf))
        fcf = fcf.view(fcf.size(0), -1)
        fcf = self.mask_fcf_3(F.relu(fcf))
        fcf = fcf.view(fcf.size(0), 1, cfg.MRCNN.RESOLUTION, cfg.MRCNN.RESOLUTION)
        #
        x = self.last_conv(x)

        return F.relu(self.upconv(x), inplace=True), fcf
