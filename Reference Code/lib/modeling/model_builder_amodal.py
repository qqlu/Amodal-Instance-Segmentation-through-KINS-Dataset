...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        ...
        # Amodal Branch
        if cfg.MODEL.AMODAL_ON:
            self.Amodal_Head = get_func(cfg.MRCNN.ROI_MASK_HEAD)(
                self.RPN.dim_out, self.roi_feature_transform, self.Conv_Body.spatial_scale)
            if getattr(self.Mask_Head, 'SHARE_RES5', False):
                self.Amodal_Head.share_res5_module(self.Box_Head.res5)
            self.Amodal_Outs = mask_rcnn_heads.mask_rcnn_outputs(self.Mask_Head.dim_out, amodal=True)
    ...
    def _forward(self, data, im_info, roidb=None, conv_body_only=False, **rpn_kwargs):
        ...
        if self.training:
            ...
            if cfg.MODEL.AMODAL_ON:
                if getattr(self.Amodal_Head, 'SHARE_RES5', False):
                    amodal_feat = self.Amodal_Head(res5_feat, rpn_ret,
                                               roi_has_mask_int32=rpn_ret['roi_has_mask_int32'])
                else:
                    amodal_feat = self.Amodal_Head(blob_conv, rpn_ret)
                amodal_pred = self.Amodal_Outs(amodal_feat)
                # print(amodal_pred.shape)
                # print(rpn_ret['masks_amodal_int32'].shape)
                # input()
                loss_amodal = mask_rcnn_heads.mask_rcnn_losses(amodal_pred, rpn_ret['masks_amodal_int32'])
    ...
    def mask_net(self, blob_conv, rpn_blob):
        ...
        if not self.training:
            ...
            mask_pred = self.Mask_Outs(mask_feat)
            amodal_feat = self.Amodal_Head(blob_conv, rpn_blob)
            amodal_pred = self.Amodal_Outs(amodal_feat)
            return mask_pred, amodal_pred
        else:
