...
class Generalized_RCNN(nn.Module):
    ...
    def _forward(self, data, im_info, roidb=None, conv_body_only=False, **rpn_kwargs):
        ...
        if not cfg.MODEL.RPN_ONLY:
            box_class_feat_last = self.Box_Class_Head(blob_conv, rpn_ret)
            if cfg.MODEL.INMODAL_ON:
                box_amodal_feat_last = self.Box_Amodal_Head(blob_conv, rpn_ret)
        ...
        if self.training:
            ...
            if cfg.MODEL.MASK_ON:
                if getattr(self.Amodal_Mask_Head, 'SHARE_RES5', False):
                    amodal_mask_feat = self.Amodal_Mask_Head(res5_feat, rpn_ret,
                                               roi_has_mask_int32=rpn_ret['roi_has_mask_int32'])
                else:
                    amodal_mask_feat = self.Amodal_Mask_Head(blob_conv, rpn_ret)
            ...
            if cfg.MODEL.INMODAL_ON:
                if getattr(self.Inmodal_Mask_Head, 'SHARE_RES5', False):
                    inmodal_mask_feat = self.Inmodal_Mask_Head(res5_feat, rpn_ret,
                                               roi_has_mask_int32=rpn_ret['roi_has_mask_int32'])
                else:
                    inmodal_mask_feat = self.Inmodal_Mask_Head(blob_conv, rpn_ret)
    ...
    def mask_net(self, blob_conv, rpn_blob):
        """For inference"""
        if not self.training:
            amodal_mask_feat = self.Amodal_Mask_Head(blob_conv, rpn_blob)
            amodal_mask_pred = self.Amodal_Mask_Outs(amodal_mask_feat)
            inmodal_mask_feat = self.Inmodal_Mask_Head(blob_conv, rpn_blob)
            inmodal_mask_pred = self.Inmodal_Mask_Outs(inmodal_mask_feat)
