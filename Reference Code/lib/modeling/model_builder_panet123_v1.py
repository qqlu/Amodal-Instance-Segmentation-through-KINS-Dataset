...
import modeling.rpn_heads_panet as rpn_heads
import modeling.fast_rcnn_heads_panet2_v1 as fast_rcnn_heads
import modeling.mask_rcnn_heads_panet23_v1 as mask_rcnn_heads
...
class Generalized_RCNN(nn.Module):
    ...
    def _forward(self, data, im_info, roidb=None, conv_body_only=False, **rpn_kwargs):
        ...
        if self.training:
            ...
            if cfg.MODEL.MASK_ON:
                if getattr(self.Mask_Head, 'SHARE_RES5', False):
                    mask_feat_fcn, mask_feat_fcf = self.Mask_Head(res5_feat, rpn_ret,
                                               roi_has_mask_int32=rpn_ret['roi_has_mask_int32'])
                else:
                    mask_feat_fcn, mask_feat_fcf = self.Mask_Head(blob_conv, rpn_ret)
                mask_pred = self.Mask_Outs(mask_feat_fcn, mask_feat_fcf)
    ...
    def mask_net(self, blob_conv, rpn_blob):
        """For inference"""
        if not self.training:
            mask_feat_fcn, mask_feat_fcf = self.Mask_Head(blob_conv, rpn_blob)
            mask_pred = self.Mask_Outs(mask_feat_fcn, mask_feat_fcf)
            return mask_pred
        else:
