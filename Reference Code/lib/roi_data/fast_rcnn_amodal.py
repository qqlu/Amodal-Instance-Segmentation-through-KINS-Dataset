...
import roi_data.mask_rcnn_amodal
...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training and cfg.MODEL.AMODAL_ON:
        # amodal
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    if cfg.MODEL.MASK_ON:
        roi_data.mask_rcnn_amodal.add_mask_rcnn_blobs(
...
def _add_multilevel_rois_panet(blobs):
    ...
    if cfg.MODEL.AMODAL_ON:
        _distribute_rois_over_fpn_levels('mask_amodal_rois')
    if cfg.MODEL.KEYPOINTS_ON:
        _distribute_rois_over_fpn_levels('keypoint_rois')
