...
import roi_data.mask_rcnn_amodalinmodalseg
...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training:
        ...
        blob_names += ['labels_amodal_int32']
        blob_names += ['fg_inds']
    if is_training:
    ...
    if is_training and cfg.MODEL.MASK_ON:
        ...
        blob_names += ['masks_amodal_int32']
    if is_training and cfg.MODEL.INMODAL_ON:
        # inmodal
        blob_names += ['masks_inmodal_int32']
...
def add_fast_rcnn_blobs_panet(blobs, im_scales, roidb):
    """Add blobs needed for training Fast R-CNN style models."""
    # Sample training RoIs from each image and append them to the blob lists
    for im_i, entry in enumerate(roidb):
        frcn_blobs = _sample_rois(entry, im_scales[im_i], im_i)
    ...
    # Add FPN multilevel training RoIs, if configured
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
        _add_multilevel_rois_panet(blobs)
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    sampled_boxes = roidb['boxes_amodal'][keep_inds]
    amodal_labels = roidb['amodal_label'][keep_inds]

    if 'bbox_targets' not in roidb:
        gt_inds = np.where(roidb['gt_classes'] > 0)[0]
        gt_boxes = roidb['boxes_amodal'][gt_inds, :]
    ...
    blob_dict = dict(
        ...
        labels_amodal_int32=amodal_labels.astype(np.int32, copy=False),
        rois=sampled_rois,
    ...
    if cfg.MODEL.MASK_ON:
        roi_data.mask_rcnn_amodalinmodalseg.add_mask_rcnn_blobs(
...
def _add_multilevel_rois_panet(blobs):
    """By default training RoIs are added for a single feature map level only.
    When using FPN, the RoIs must be distributed over different FPN levels
    according the level assignment heuristic (see: modeling.FPN.
    map_rois_to_fpn_levels).
    ...
    def _distribute_rois_over_fpn_levels(rois_blob_name):
        ...
        # )
        # Add per FPN level roi blobs named like: <rois_blob_name>_fpn<lvl>
        fpn_utils.add_multilevel_roi_blobs_panet(
            blobs, rois_blob_name, blobs[rois_blob_name], lvl_min, lvl_max
        )
