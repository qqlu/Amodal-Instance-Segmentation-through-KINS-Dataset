...
class RoiDataLoader(data.Dataset):
    ...
    def __getitem__(self, index_tuple):
        ...
        # print(blobs['roidb'][0]['segms_amodal'])
        # input()
        # pdb.set_trace()
        ...
        if self._roidb[index]['need_crop']:
            ...
            # Check bounding box
            entry = blobs['roidb'][0]
            boxes = entry['boxes_amodal']
            invalid = (boxes[:, 0] == boxes[:, 2]) | (boxes[:, 1] == boxes[:, 3])
            valid_inds = np.nonzero(~ invalid)[0]
            if len(valid_inds) < len(boxes):
                for key in ['boxes_amodal', 'gt_classes', 'segms_amodal_areas', 'segms_invisible_full', 
                            'gt_overlaps', 'is_crowd', 'box_to_gt_ind_map', 'gt_keypoints']:
                    if key in entry:
                        entry[key] = entry[key][valid_inds]
                entry['segms_amodal'] = [entry['segms_amodal'][ind] for ind in valid_inds]
                entry['segms_inmodal'] = [entry['segms_inmodal'][ind] for ind in valid_inds]
