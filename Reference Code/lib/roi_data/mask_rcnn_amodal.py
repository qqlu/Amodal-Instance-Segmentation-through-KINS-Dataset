...
def add_mask_rcnn_blobs(blobs, sampled_boxes, roidb, im_scale, batch_idx):
    ...
    if type(polys_gt[0]) == list:
        polys_flag = True
    else:
        assert type(polys_gt[0]) == dict
        polys_flag = False
    
    if polys_flag:
        boxes_from_polys = segm_utils.polys_to_boxes(polys_gt)
    else:
        rle_masks_gt = [np.array(mask_util.decode(rle), dtype=np.float32) for rle in polys_gt]
        boxes_from_polys, _ = segm_utils.rle_masks_to_boxes(rle_masks_gt)

    rle_masks_gt_amodal = [np.array(mask_util.decode(rle), dtype=np.float32) for rle in polys_gt_amodal]
    ...
    if fg_inds.shape[0] > 0:
        ...
        if polys_flag:
            for i in range(rois_fg.shape[0]):
                ...
                poly_gt_amodal = rle_masks_gt_amodal[fg_polys_ind]

                roi_fg = rois_fg[i]
                # Rasterize the portion of the polygon mask within the given fg roi
                # to an M x M binary image
                mask = segm_utils.polys_to_mask_wrt_box(poly_gt, roi_fg, M)
                mask_amodal = segm_utils.rle_mask_to_mask_wrt_box_2(poly_gt_amodal, roi_fg, M)

                # mask = segm_utils.rle_to_mask_wrt_box(poly_gt, roi_fg, M)
                mask = np.array(mask > 0, dtype=np.int32)  # Ensure it's binary
                mask_amodal = np.array(mask_amodal > 0, dtype=np.int32)
                masks[i, :] = np.reshape(mask, M**2)
                masks_amodal[i, :] = np.reshape(mask_amodal, M**2)
        else:
            for i in range(rois_fg.shape[0]):
                fg_polys_ind = fg_polys_inds[i]
                poly_gt = rle_masks_gt[fg_polys_ind]
                poly_gt_amodal = rle_masks_gt_amodal[fg_polys_ind]
                roi_fg = rois_fg[i]
                # Rasterize the portion of the polygon mask within the given fg roi
                # to an M x M binary image
                mask = segm_utils.rle_mask_to_mask_wrt_box_2(poly_gt, roi_fg, M)
                mask_amodal = segm_utils.rle_mask_to_mask_wrt_box_2(poly_gt_amodal, roi_fg, M)
                # mask = segm_utils.rle_to_mask_wrt_box(poly_gt, roi_fg, M)
                mask = np.array(mask > 0, dtype=np.int32)  # Ensure it's binary
                mask_amodal = np.array(mask_amodal > 0, dtype=np.int32)
