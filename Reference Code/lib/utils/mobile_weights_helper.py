...
def load_pretrained_imagenet_weights(model):
    ...
    if ext == '.pkl':
        ...
        for name, mod in model.named_modules():
            if isinstance(mod, mynn.AffineChannel2d):
                # if cfg.FPN.FPN_ON:
                #     k_name = name.split('.', 2)[-1]
                #     print(name)
                # else:
                #     k_name = name.split('.', 1)[-1]
                pretrianed_name = name_mapping[name+'.weight']
                pretrianed_name = pretrianed_name[:-7]
    ...
    pattern = dwh.mobile_weights_name_pattern()

    name_mapping, _ = model.detectron_weight_mapping
    # print(pretrianed_state_dict.keys())
        ...
        if isinstance(v, str):  # maybe a str, None or True            
            if pattern.match(v):
                ...
                if ext == '.pkl':
                    ...
                    model_state_dict[k].copy_(pretrianed_state_dict[v])
