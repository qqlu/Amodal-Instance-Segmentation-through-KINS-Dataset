...
def rle_mask_to_mask_wrt_box(gt_mask, ex_box, M):
  #w = box[2] - box[0]
  #h = box[3] - box[1]
  #w = np.maximum(w, 1)
  #h = np.maximum(h, 1)
  #gt_mask = np.array(mask_util.decode(rle), dtype=np.float32)
  ...
  ex_box_1 = [int(ex_box[0]), int(ex_box[1]), int(ex_box[2]), int(ex_box[3])]
  box_mask = intersect_mask(ex_box_1, gt_box, gt_mask, M)
  return box_mask

def rle_mask_to_mask_wrt_box_2(mask, box, M):
    #mask = np.array(mask_util.decode(rle), dtype=np.float32)
    sx = max(0, (box[2] - box[0]) / M)
    sy = max(0, (box[3] - box[1]) / M)
    matrix = np.array([[sx, 0, box[0]], [0, sy, box[1]]], dtype=np.float)
    mask = cv2.warpAffine(mask, matrix, (M, M), flags=cv2.WARP_INVERSE_MAP + cv2.INTER_NEAREST, borderMode=cv2.BORDER_REFLECT)
    return mask

    

def intersect_mask(ex_box, gt_box, gt_mask, M):
    """
    ...
    regression_target = cv2.resize(regression_target, (M, M))
    regression_target = regression_target >= 0.4
    return regression_target.astype(np.float32)
...
def rle_masks_to_boxes(decoded_masks):
